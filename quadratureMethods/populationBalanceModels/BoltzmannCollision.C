...
)
{
    ...
    forAll(gPow, powi)
    {
        omegaPow[powi] = pow(omega, (const int)powi);
        for (label cmpt = 0; cmpt < 3; cmpt++)
        {
            gPow[powi][cmpt] = pow(g[cmpt], (const int)powi);
            vPow[powi][cmpt] = pow(u1[cmpt], (const int)powi);
        }
    }
                    ...
                    forAll(scalarIndexes_, cmpt)
                    {
                        scalar absCmpt =
                            ...
                            (
                                ...
                                (const int)momentOrder[scalarIndexes_[cmpt]]
                            );
